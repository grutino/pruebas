...
800 IF CX=R AND CY<=G(R)+2 THEN LET WON=1: LET EXPX=CX: LET EXPY=G(R)+1: LET M$="IMPACTO DIRECTO!": LET DONE=1: GO TO 900
810 IF CY<=G(CX) THEN LET EXPX=CX: LET OLD=G(CX): LET EXPY=OLD+1: LET CR=INT (1+RND*2): LET G(CX)=OLD-CR: IF G(CX)<0 THEN LET G(CX)=0: LET M$="EXPLOSION EN LA MONTAÑA": LET DONE=1: GO TO 900
820 NEXT TM
830 GO TO 910
